...
#include "PartingOperationPresenter.h"
#include <spdlog/spdlog.h>

#include "GeometryUtils.h"
#include "Line.h"

PartingOperationPresenter::PartingOperationPresenter(const MachineConfig &machineConfig, const ToolTable &toolTable, const Project &project, GeometryView &geometryView, OperationConfigurationView &operationConfigView, QObject *parent) : OperationConfigurationPresenter(visibility, machineConfig, toolTable, project, geometryView, operationConfigView, parent) {
    operationConfig.operationType = OperationType::Parting;
}

void PartingOperationPresenter::onSegmentSelected(size_t segmentIndex) {
    ...
    if (auto line = dynamic_cast<Line*>(project.geometry.segments[segmentIndex].get())) {
        if (!line->isVertical()) {
            spdlog::warn("Parting operation requires a vertical line segment. Selected segment is not vertical.");
            return;
        }
        ...
        spdlog::warn("Parting operation requires a line segment. Selected segment is not a line.");
        return;
    }
}
